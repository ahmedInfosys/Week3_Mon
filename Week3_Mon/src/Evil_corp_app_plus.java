		...
		while(true){
			...
			case "c"://c letter for checking.
				...
				for(int i = 0; i < list_of_accounts.size(); i++){
					...
					System.out.printf("%-20s%-15s%-15s\n", "------------------", "--------------", "-----------");
					for(int j = 0; j <= i; j++){
						System.out.printf("%-20s%-15s%-15s\n", list_of_accounts.get(j).getName(),list_of_accounts.get(j).getAccount_number(),list_of_accounts.get(j).getBalance());
					}
					if(list_of_accounts.get(i).getAccount_number() == account_number)
					{
						...
						for(String KEY:list_of_accounts.get(i).get_info().keySet()){
							...
							writer.println(sdf.format(list_of_accounts.get(i).get_info().get(KEY).getTime()) + "\t\t" + KEY);
						}
					}
